...
function SavedMovies(props) {
    ...
    mainApi
      ...
      .then((res) => {
        ...
        localStorage.setItem(
          "savedMovies",
          JSON.stringify(ownersCardsFromCommonSavedValue)
        );
  ...
  function sortCards(value) {
    const sorted = value.filter((item) => {
      if (
        ...
        item.nameRU
          .toLowerCase()
          .includes(localStorage.getItem("localMovieName"))
      ) {
        return item;
      }
      if (
  ...
  async function findInSavedMovies() {
    ...
    (await !isShortFilm) &&
      props.setSavedMovies(
        sortCards(JSON.parse(localStorage.getItem("savedMovies")))
      );
    await [
      !sortCards(JSON.parse(localStorage.getItem("savedMovies"))).length
        ? setMovieNotFound(true)
        : setMovieNotFound(false),
      setBackToSavedMovies(true),
    ];
  ...
  function deleteMovieHandler(card) {
    mainApi
      ...
      .then((res) => {
        const checked = JSON.parse(localStorage.getItem("savedMovies")).filter(
          (movie) => !res.message.includes(movie.nameRU)
        );
        const deleted = props.savedMovies.filter((movie) =>
  ...
  return (
    <div className="saved-movies">
      <SearchForm
        getMovieHandler={findInSavedMovies}
