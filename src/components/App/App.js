...
function App() {
  ...
  function handleSignUp({ name, password, email }) {
    auth
      .signUp({ name, password, email: email.toLowerCase() })
      .then((res) => {
  ...
  return (
    <CurrentUserContext.Provider value={isLogged ? currentUser : ""}>
      <div className="page">
        {isModalOpen && (
          <ModalPopup
            closeModal={hideServerErrorHandler}
        ...
        <div
          className={`page__container ${
            isMenuOpen && screenWidth < 769 ? "page__container_dark" : ""
          }`}
        >
          {location.pathname !== "/signin" &&
            location.pathname !== "/signup" && (
              <Header
                menuHandler={menuOpenHandler}
