...
function Movies(props) {
  ...
  // изменение чекбокса назначим чтоб брал значение из локала
  ...
  function sortCards(value) {
    const sorted = value.filter((item) => {
      if (
        item.nameRU &&
        item.nameRU.toLowerCase().includes(localStorage.getItem("movieName"))
      ) {
        return item;
      }
      if (
        item.nameEN &&
        item.nameEN.toLowerCase().includes(localStorage.getItem("movieName"))
      ) {
        return item;
      }
    });
      ...
      for (let i in item) {
        ...
        if (i === "trailerLink") {
          if (!isURL(item[i])) {
            item[
              i
            ] = `https://www.youtube.com/results?search_query=${item.nameEN.replaceAll(
              " ",
              "+"
            )}`;
          }
  ...
  function deleteMovieHandler(card) {
    mainApi
      ...
      .then((res) => {
        const deleted = props.movies.filter((movie) =>
          res.message.includes(movie.nameRU)
        );
        const checked = props.movies.filter(
          (movie) => !res.message.includes(movie.nameRU)
        );
        const localMovies = JSON.parse(localStorage.getItem("movies"));

        localMovies.forEach((movie) => {
          if (
            movie.id === deleted[0].id ||
            movie.id === Number(deleted[0].movieId)
          ) {
            movie.saved = false;
            movie._id = "";
          }
  ...
  async function getMovieHandler() {
    if (beatFilmBase) {
      ...
      const checkedValidityMovies = await checkIncomingValueValidityHandler(
        sorted
      );
    ...
    try {
      ...
      const currentUsersSavedCards = await separateCurrentUsersSavedMovies(
        allSavedMoviesByUsers.data
      );
      await [
        props.setSavedMovies(currentUsersSavedCards),
        localStorage.setItem(
          "savedMovies",
          JSON.stringify(currentUsersSavedCards)
        ),
      ];
      const movieApiFilms = await movieApi.getMovies();
      const checkedmovieApiData = await checkIncomingValueValidityHandler(
        movieApiFilms
      );
      await localStorage.setItem(
        "beatFilmBase",
        JSON.stringify(checkedmovieApiData)
      );
  ...
  function moreButtonVisibilityHandler() {
    const status =
      currentCardArrayLength !== 0 &&
      props.movies.length &&
      !(props.movies.length === currentCardArrayLength);
